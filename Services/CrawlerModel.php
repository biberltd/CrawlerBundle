...
class CrawlerModel extends CoreModel
{
    ...
    public function __construct($kernel, $db_connection = 'default', $orm = 'doctrine')
    {
        ...
        $this->entity = array(
            ...
            'clr' => array('name' => 'CrawlerBundle:CrawlerRequest', 'alias' => 'clr'),
     ...
     * @author          Said İmamoğlu
     *
     * @use             $this->deleteCrawlerRequests()
     *
     * @param           mixed $crawlerRequest
     *
     * @return          \BiberLtd\Bundle\CoreBundle\Responses\ModelResponse
     */
    public function deleteCrawlerRequest($crawlerRequest)
    {
        return $this->deleteCrawlerRequests(array($crawlerRequest));
    }

    /**
     * @name            deleteCrawlerRequests ()
     *
     * @since            1.0.0
     * @version         1.0.0
     * @author          Said İmamoğlu
     ...
     * @return          \BiberLtd\Bundle\CoreBundle\Responses\ModelResponse
     */
    public function deleteCrawlerRequests($collection)
    {
        $timeStamp = time();
        if (!is_array($collection)) {
            return $this->createException('InvalidParameterValueException', 'Invalid parameter value. Parameter must be an array collection', 'E:S:001');
        ...
        $countDeleted = 0;
        foreach ($collection as $entry) {
            if ($entry instanceof BundleEntity\CrawlerRequest) {
                $this->em->remove($entry);
                $countDeleted++;
            } else {
                $response = $this->getCrawlerRequest($entry);
                if (!$response->error->exists) {
                    $entry = $response->result->set;
                    $this->em->remove($entry);
                    $countDeleted++;
     ...
     * @use             $this->createException()
     *
     * @internal param mixed $crawlerLink
     *
     */
    public function getCrawlerRequest($crawlerRequest)
    {
        $timeStamp = time();
        if ($crawlerRequest instanceof BundleEntity\CrawlerRequest) {
            return new ModelResponse($crawlerRequest, 1, 0, null, false, 'S:D:002', 'Entries successfully fetched from database.', $timeStamp, time());
        }
        $result = null;
        switch ($crawlerRequest) {
            case is_numeric($crawlerRequest):
                $result = $this->em->getRepository($this->entity['clr']['name'])->findOneBy(array('id' => $crawlerRequest));
                break;
            case is_string($crawlerRequest):
                $result = $this->em->getRepository($this->entity['clr']['name'])->findOneBy(array('url' => $crawlerRequest));
                break;
        }
        if (is_null($result)) {
            return new ModelResponse($result, 0, 0, null, true, 'E:D:002', 'Unable to find request entry in database.', $timeStamp, time());
     ...
     * @use             $this->insertCrawlerLinks()
     *
     * @internal param mixed $crawlerLink Entity or post
     *
     */
    public function insertCrawlerRequest($crawlerRequest)
    {
        return $this->insertCrawlerRequests(array($crawlerRequest));
    }
     ...
     * @return          array           $response
     */
    public function insertCrawlerRequests($collection)
    {
        $timeStamp = time();
        /** Parameter must be an array */
        if (!is_array($collection)) {
        ...
        foreach ($collection as $data) {
            if ($data instanceof BundleEntity\CrawlerRequest) {
                $entity = $data;
                if(null == $entity->getDateAdded()){
                    $entity->setDateAdded($now);
                }
                if(null == $entity->getDateUpdated()){
                ...
                $entity = new BundleEntity\CrawlerRequest();
                if (!property_exists($data, 'date_added')) {
                    $data->date_added = $now;
                }
                if (!property_exists($data, 'date_updated')) {
     ...
     * @author          Said İmamoğlu
     *
     * @use             $this->updateCrawlerRequests()
     *
     * @param           mixed $crawlerRequest
     *
     * @return          mixed           $response
     */
    public function updateCrawlerRequest($crawlerRequest)
    {
        return $this->updateCrawlerRequests(array($crawlerRequest));
    }

    /**
     * @name            updateCrawlerRequests ()
     *
     * @since            1.0.0
     * @version         1.0.0
     * @author          Said İmamoğlu
     ...
     * @return          array           $response
     */
    public function updateCrawlerRequests($collection)
    {
        $timeStamp = time();
        /** Parameter must be an array */
        if (!is_array($collection)) {
        ...
        foreach ($collection as $data) {
            if ($data instanceof BundleEntity\CrawlerRequest) {
                ...
                $response = $this->getCrawlerRequest($data->id);
                if ($response->error->exist) {
                    return $this->createException('EntityDoesNotExist', 'CrawlerRequest with id ' . $data->id . ' does not exist in database.', 'E:D:002');
                }
                $oldEntity = $response->result->set;
                foreach ($data as $column => $value) {
                    $set = 'set' . $this->translateColumnName($column);
     ...
     */
    public function listCrawlerRequests($filter = null, $sortOrder = null, $limit = null)
    {
        $timeStamp = time();
        if (!is_array($sortOrder) && !is_null($sortOrder)) {
            return $this->createException('InvalidSortOrderException', '$sortOrder must be an array with key => value pairs where value can only be "asc" or "desc".', 'E:S:002');
        ...
        $qStr = 'SELECT ' . $this->entity['clr']['alias']
            . ' FROM ' . $this->entity['clr']['name'] . ' ' . $this->entity['clr']['alias'];
        ...
        if (!is_null($sortOrder)) {
            foreach ($sortOrder as $column => $direction) {
                switch ($column) {
                    ...
                    case 'date_failed':
                    case 'date_completed':
                        $column = $this->entity['clr']['alias'] . '.' . $column;
                        break;
                }
                $oStr .= ' ' . $column . ' ' . strtoupper($direction) . ', ';
            }
     ...
     * @internal param $site
     */
    public function listWaitingCrawlerRequests($sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_started', 'comparison' => 'isnull', 'value' => ''),
                ),
            )
        );
        $filter[] = array(
            ...
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_failed', 'comparison' => 'isnull', 'value' => ''),
                ),
            )
        );
        $filter[] = array(
            ...
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_completed', 'comparison' => 'isnull', 'value' => '' ),
                ),
            )
        );

        return $this->listCrawlerRequests($filter,$sortOrder,$limit);
    }
    /**
     * @param $site
     * @param null $sortOrder
     ...
     * @return array
     */
    public function listWaitingCrawlerRequestsOfSite($site,$sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.site', 'comparison' => '=', 'value' => $site),
                ),
            )
        );
     ...
     * @return array
     */
    public function listActiveCrawlerRequestsOfSite($site,$sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
     ...
     * @return array
     */
    public function listRunningCrawlerRequestsOfSite($site,$sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
        ...
        $filter[] = array(
            ...
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_started', 'comparison' => 'isnotnull', 'value' => ''),
                ),
            )
        );
        $filter[] = array(
     ...
     * @return array
     */
    public function listFailedCrawlerRequestsOfSite($site,$sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
        ...
        $filter[] = array(
            ...
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_failed', 'comparison' => 'isnotnull', 'value' => ''),
                ),
            )
        );
        $filter[] = array(
     ...
     * @return array
     */
    public function listSuccessfullCrawlerRequestsOfSite($site,$sortOrder = null,$limit = null){
        $filter[] = array(
            'glue' => 'and',
            'condition' => array(
                array(
        ...
        $filter[] = array(
            ...
            'condition' => array(
                array(
                    'glue' => 'and',
                    'condition' => array('column' => 'clr.date_completed', 'comparison' => 'isnotnull', 'value' => '' ),
                ),
            )
        );
